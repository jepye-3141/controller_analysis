...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
Highly unstable initial states, such as those produced by ballistic deployment, require nonlinear controllers to stabilize the tumble before the vehicle crashes. We derive and test two nonlinear controllers for which we establish Lyapunov-based stability conditions: a continuous-time Sliding Mode Controller (cSMC) and a discrete-time Sliding Mode Controller (dSMC). These conditions govern the four actuated channels (altitude, roll, pitch, and yaw); the cSMC guarantee is continuous-time and does not survive discretization, whereas the dSMC is built directly in discrete time and holds under the discretized plant. We test these against two off-the-shelf controllers: the Linear Quadratic Regulator with Integral Action (LQR) and the cascaded Proportional-Integral-Derivative (PID) controller. Sampling 17 deployment states along one ballistic trajectory, we find the dSMC recovers more of them than the LQR, PID, or continuous-time controllers. We then extend the dSMC to account for possible motor control constraints in terms of maximum squared rotational speed. If the desired motor speeds fall within the physical bounds, then the augmented controller and the ideal controller are identical, and we satisfy the exponential reaching law. Otherwise, we saturate the control inputs in order of increasing importance---yaw first, then altitude/thrust, then pitch/roll---and obtain a weaker input-to-state stability (ISS) bound that holds while the trajectory stays within an explicit envelope. However, despite this weaker stability, on a separate 140-trial landing sweep the priority-weighted saturation raises average landing-site reachability roughly $2.7\times$ over simple per-motor clipping, from $22.1\%$ to $60.7\%$.
\end{abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
= \begin{bmatrix} 6 \\ 10 \\ 10 \\ 0.5 \end{bmatrix}.
\end{aligned}
\end{equation}

\begin{align}
\hat{u}_T
&= -\frac{m\,c_T}{\cs\theta\,\cs\phi}\bigl(v_x\,\s\theta - v_y\,\cs\theta\,\s\phi - v_z\,\cs\theta\,\cs\phi\bigr) \notag \\
...
\dot s_T
&= c_T\!\bigl(
v_x \s\theta - v_y \cs\theta \s\phi - v_z \cs\theta \cs\phi
\bigr) \\
&\quad - \Bigl(
...
So, $\dot V_\alpha(x)$ is negative definite, $V_\alpha(x)$ is positive definite, and $V_\alpha(0)=0$, and the Lyapunov stability requirements are satisfied for $V_\alpha$ in Equation \ref{eq:lyapunov}. The surfaces $s_\alpha$ thus decay to zero over time, which, from Equation \ref{eq:smc-defs}, is equivalent to the tracking error decaying to zero. The certified error $e_\alpha$ is that of the four actuated channels ($z, \phi, \theta, \psi$), with the outer reference $r_{PID}$ held fixed while differentiating $s_\alpha$; horizontal $(x,y)$ tracking is delegated to the outer position loop and is not covered by this inner-loop certificate. Additionally, within the boundary layer ($|s_\alpha| < 1/2$), $\sat(2s_\alpha,-1,1) = 2s_\alpha$, and so
...
From here, our work diverges significantly from \cite{refs:Xiong2016}. We derive different control laws for $z$ and $\psi$, as the originally proposed control laws do not satisfy the reaching conditions stated above. We assume that the quadrotor is symmetric in the rotor plane, so $J_{xx} = J_{yy}$, as for almost all commercial quadrotors. This inertial symmetry is load-bearing in the yaw channel: it removes the $(J_{xx}-J_{yy})\,\omega_x\omega_y$ gyroscopic term from Euler's equation, leaving $\dot\omega_z = M_z/J_{zz}$ (Equation~\ref{eq:nonlinear-eom}), on which the $\psi$ sliding law of Equation~\ref{eq:spsi-wrap-tighter} is built. The roll and pitch laws, the deficit-coupling matrix $D_k$, and the ISS bound retain $J_{xx}$ and $J_{yy}$ as distinct parameters; an asymmetric vehicle would reintroduce the yaw cross-term but leave the structure of the rest of the analysis unchanged.
...
In implementation, the fully-actuated sliding variables are saturated to a band $s_{z,k},\, s_{\psi,k} \in [-c,\, c]$ before entering the control laws, with $c = 2$ for the baseline formulation and $c = 1.578$ for the tuned saturated formulation (Table~\ref{tab:gains-scalars}). Outside the band the reaching step becomes the constant-rate $\Delta_{k+1,k} s_i = -c\,\eta_i\,\Delta t\,\sgn(s_{i,k})$, which still satisfies the discrete existence condition $|s_{i,k+1}| < |s_{i,k}|$ for $\eta_i\,\Delta t < 2$; the exponential reaching law above is recovered once $|s_{i,k}| \leq c$. In the saturated controller of Section~\ref{sec:sat-aux}, the clip is applied to $s$ before the auxiliary-state correction that forms $\tilde s$.
...
\vspace*{25pt}
\centering
\caption{Gains and scalar parameters. Reaching-law gains, sliding-surface coefficients and the sliding-variable clip are those of the saturated dSMC, set by a global Latin-hypercube search over landing reachability; the parenthesized value is the untuned baseline set that the no-saturation formulation still runs.}
...
$\eta_z$ & $7.3044$ $(7)$ & $\eta_\psi$ & $8.2874$ $(7)$ \\
$\eta_\phi$ & $4.9968$ $(14)$ & $\eta_\theta$ & $4.9968$ $(14)$ \\
$\alpha_z$ & $10.4815$ $(6)$ & $\alpha_\psi$ & $3.1535$ $(1)$ \\
$b$ & $5$ & $d$ & $2$ \\
$\Omega^2_{\min}$ & $0$ & $\Omega^2_{\max}$ & $2$ \\
$s_z, s_\psi$ clip & $\pm 1.578$ $(\pm 2)$ & $k_{\xi,\alpha}$ & $1.2741\,\eta_\alpha\,\Delta t$ \\
\bottomrule
\end{tabular}
\end{table}
\endgroup
...
\begin{table}[t]
\centering
\caption{Coefficient definitions $a_i$, for the saturated dSMC. The untuned baseline formulation uses $6$, $2$, $2$ and $8$ in place of the four leading constants, mirrored in $a_5$--$a_8$.}
\label{tab:a-coeffs}
\begin{tabular}{ll@{\hskip 18pt}ll}
\toprule
$a_i$ & Definition & $a_i$ & Definition \\
\midrule
$a_1$ & $\displaystyle \frac{5.1762\,m}{u_T\,\cs\psi}$
& $a_2$ & $\displaystyle \frac{0.7410\,m}{u_T\,\cs\psi}$ \\
$a_3$ & $3.4330$ & $a_4$ & $14.6286$ \\
$a_5$ & $\displaystyle -\frac{5.1762\,m}{u_T\,\cs\phi\,\cs\psi}$
& $a_6$ & $\displaystyle -\frac{0.7410\,m}{u_T\,\cs\phi\,\cs\psi}$ \\
$a_7$ & $3.4330$ & $a_8$ & $14.6286$ \\
\bottomrule
\end{tabular}
\end{table}
\endgroup
...
At evenly spaced points along the ballistic trajectory (Figure~\ref{fig:ballistic_trajectories}), we sampled the vehicle state and ran a 30-second stabilization attempt from each sample, recording which initial conditions each controller could recover from. Each shell state maps to the drone's 12-state initial condition by aligning the body $x$-axis with the pointing vector (zero roll) and rotating the shell's linear and angular velocity into that body frame. A trial was counted as successful if the vehicle survived the full stabilization window without the divergence guard firing and satisfied the three conditions below:
\begin{equation}
...
\end{equation}
The first condition bounds the final horizontal position error to within $10\,\text{m}$ of the target. The second prevents the controller from accelerating the vehicle past twice its initial deployment speed $|v_0|_2$. The third caps the Euler-angle rate $\dot\Theta_t=[\dot\theta\,\dot\phi\,\dot\psi]$ at twice $|[\pi\,\pi\,\pi]^{\top}|_2 = \pi\sqrt{3} \approx 5.44\,\text{rad/s}$, the magnitude of a rate vector with $\pi\,\text{rad/s}$ on each axis. The rotational-rate bound is evaluated only after the first second of flight: the vehicle is released tumbling, so rate excursions inside that window reflect the ballistic-separation transient the controller is still arresting, not the controller itself. Of these conditions only the velocity bound can be re-scored from stored trial data without re-simulating, since it enters as the single inequality $|v_t|_2/|v_0|_2 \le V$ evaluated on a logged trace; re-scoring every dSMC trial at $V \in \{2,\,3,\,5,\,10,\,\infty\}$ leaves all four of the counts we report unchanged---$6$ and $9$ of $17$ on this envelope, and $31$ and $85$ of $140$ on the landing sweep of Section~\ref{sec:sat-validation}. The largest speed ratio reached by any trial that satisfies the other conditions is $1.40$, so no trial sits near the threshold and the reported separations do not depend on where it is placed. That insensitivity is in part a property of the plant: the simulated vehicle integrates rigid-body dynamics under gravity and thrust with no aerodynamic drag, so no terminal velocity arises on its own, failures diverge instead of settling at some large speed, and this condition stands in for the speed limit a real airframe would impose. The drag coefficients retained in the implemented thrust and yaw laws ($K_3$ and $K_6$ of Table~\ref{tab:gains-scalars}) accordingly compensate a term the simulated plant does not contain; at these values that correction is orders of magnitude below the gravitational term and does not affect the results reported here. The criterion is scored on horizontal position, whereas our stability analysis certifies the four actuated sliding variables (altitude, roll, pitch, and yaw); horizontal $(x,y)$ motion rides on the underactuated attitude channels, and its convergence is shown empirically here, not proven. Figure \ref{fig:ballistic_envelope} shows that no controller recovers every sampled state; the dSMC recovers the largest share (the saturated variant, 9 of 17 sampled states), concentrated on the descending leg past apogee. LQR and PID operate too far from their hover linearization, and cSMC’s continuous-time stability argument does not transfer to the sampled-data implementation. Each baseline uses gains tuned for stable hover and step tracking (Tables~\ref{tab:pid-gains} and \ref{tab:smc-pid-gains}, and the LQR weights of Equation~\ref{eq:lqr-weights}), not detuned to favor the dSMC; the failures here are structural---a linearization or continuity assumption broken by the tumbling deploy---rather than an artifact of gain choice. We did not exhaustively search each baseline's gain space, so we report the separation as observed on this envelope rather than as a gain-independent ranking.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\linewidth]{figs/39_ballistic_envelope.png}
\caption{Ballistic stabilization at test points along the trajectory. Recovered states per controller (of 17 sampled): LQR 5, PID 0, cSMC 0, dSMC without saturation 6, dSMC with priority-weighted saturation 9. The two dSMC formulations differ in gain set as well as saturation handling (Table~\ref{tab:gains-scalars}), so the 6-to-9 margin is not attributable to the saturation handler alone. Isolating the handler at matched tuned gains and the same $\Omega^2_{\max} = 2$, simple per-motor clipping recovers 3 of 17 and the priority-weighted allocation 9 of 17; the allocation gives up no state that clipping recovers.}
...
\end{equation}
\normalsize
returning $0$ if any component with $\text{dir}_i = 0$ has $\text{base}_i \notin [\Omega^2_{\min},\, \Omega^2_{\max}]$. The implementation additionally tracks the lower interval endpoints contributed by the opposite box face and returns $0$ when the per-component intervals for the scale have empty intersection, so an unreachable stage yields a zero scale rather than an unguaranteed one. When the base point is itself feasible, the two forms coincide and the point returned by Equation~\ref{eq:sat-maxscale} is feasible; when it is not, the returned scale carries no such guarantee. The trailing feasibility checks in the cascade below are therefore load-bearing rather than redundant: each stage short of the terminal fallback returns its candidate only after re-checking the full motor-space point.

We apply cascaded saturation by successively saturating channels, in ascending order of importance, until we reach a feasible control effort. So, we accept relatively minor errors in exchange for controlling more important channels. 
...
\end{equation}
\normalsize
With $\alpha_{M_z} = \lambda_{\max}(\text{base},\, v_{M_z})$, return $\bar u_k = (T,\, M_x,\, M_y,\, \alpha_{M_z}\, M_z)^T$ if feasible. In this regime, we begin to sacrifice some yaw control, but retain altitude and attitude control.
...
\end{equation}
which is the same rate at which the unconstrained reaching law drives $s$ to zero. This way, we can reuse the dSMC gains directly via Equation~\ref{eq:sat-matched}, as in the zero-control-deficit case, we recover the unsaturated dSMC controller. Both blocks of the closed-loop matrix $\mathcal{A}_k$ (Equation~\ref{eq:sat-closed-loop}) then decay at consistent per-channel rates, and the steady-state residual analysis of Section~\ref{sec:sat-steadystate} becomes $\Delta t$-independent ratios. The implementation scales all four channels uniformly, $k_{\xi\,\alpha} = 1.2741\,\eta_\alpha\,\Delta t$ (Table~\ref{tab:gains-scalars}), the scale factor having been carried through the same tuning search as the gains. The certificates below do not require the matched choice: they are stated for any diagonal $K_\xi$ satisfying Equation~\ref{eq:sat-xi-update}, the one bound in which $K_\xi$ and $E$ separate is evaluated at the implemented scaling (Equation~\ref{eq:sat-nu2-bound}), and the residual ratios of Section~\ref{sec:sat-steadystate} are independent of $K_\xi$ altogether. When the commanded per-motor speeds lie within the feasible range, $\Delta u_k = 0$ forces $\xi$ to contract to zero, and the modified sliding variable reduces to the baseline $s_k$ (Section~\ref{sec:sat-reduction}).

\subsection{Deficit-Coupling Matrix}
\label{sec:sat-deficit}

We now derive the coupling matrix $D_k$ between the auxiliary state and the control deficit. Two related coefficients enter the analysis and must be distinguished. Define
\begin{align}
\label{eq:sat-A-def}
A_{ij} &\triangleq \frac{\partial (\Delta s_i)}{\partial u_j}
&&\text{(formula coefficient: how commanded $u_j$ enters $\Delta s_i$)} \\
\label{eq:sat-C-def}
C_{ij} &\triangleq \frac{\partial (\Delta s_i)}{\partial (\Delta u_j)}
&&\text{(deficit-response coefficient: how the deficit propagates)}.
\end{align}
...
\end{equation}
\normalsize
With $s_{z,k} = \alpha_z(z_k^d - z_k) + (\dot z_k^d - \dot z_k)$ and constant reference state inputs, the increment is:
\small
\begin{equation}
\Delta_{k+1,k} s_z
...
\end{equation}
\normalsize
With a constant reference state, the surface evolution expands as
\small
\begin{equation}
\Delta_{k+1,k} s_\phi
...
\end{equation}
As in the unconstrained law, this identity holds inside the clip band $|s_{z,k}|,\, |s_{\psi,k}| \leq 1.578$ (Table~\ref{tab:gains-scalars}); outside it, the constant-rate reaching step drives the fully-actuated variables into the band, and the certificates below apply once inside. The actually-applied input is $\bar u_k = u_k - \Delta u_k$. By Equation~\ref{eq:sat-C-equals-minusA},
\begin{equation}
...
\end{equation}
\normalsize
The first Young term carries the quadratic form $(I-E)^T W_s (I-E)$, not the decay term~(a), so its per-channel coefficient is $+\mu\,(1 - \eta_\alpha \Delta t)^2\, (W_s)_{\alpha\alpha}$. Combining it with (a) gives the net diagonal coefficient $-\bigl[\eta_\alpha \Delta t\,(2 - \eta_\alpha \Delta t) - \mu\,(1 - \eta_\alpha \Delta t)^2\bigr](W_s)_{\alpha\alpha}$ on each $\tilde s_\alpha^2$, which is negative definite iff the bracket is positive on every channel:
\begin{equation}
\label{eq:sat-mu-bound}
0 < \mu < \min_\alpha \frac{\eta_\alpha \Delta t\,(2 - \eta_\alpha \Delta t)}{(1 - \eta_\alpha \Delta t)^2}.
\end{equation}
Hence $\mu \in (0,1)$ alone is insufficient: the absorbed form scales as $(1-\eta_\alpha\Delta t)^2$, not $\eta_\alpha\Delta t\,(2-\eta_\alpha\Delta t)$. For the saturated formulation's gains of Table~\ref{tab:gains-scalars} at $50$~Hz ($\eta_z\Delta t = 0.146$, $\eta_\psi\Delta t = 0.166$, $\eta_\phi\Delta t = \eta_\theta\Delta t = 0.100$), the binding $(\phi,\theta)$ channels require $\mu < 0.234$. The second Young term adds to the cross-square residual~(c).
...
\end{equation}
Under the matched gains of Equation~\ref{eq:sat-matched}, $k_{\xi\,\alpha} = \eta_\alpha \Delta t$ and this bound coincides with Equation~\ref{eq:sat-mu-bound}. The implemented $1.2741$ scaling separates the two: at $50$~Hz it gives $\nu_2 < 0.313$ against $\mu < 0.234$, both binding on the $(\phi,\theta)$ channels. The second piece adds to the deficit penalty (f). We fix $\mu$ and $\nu_2$ within Equations~\ref{eq:sat-mu-bound} and \ref{eq:sat-nu2-bound} before selecting $\rho$ in the next step, because the domination condition there must hold against the $\xi$-decay margin that survives this erosion.

\item With $\mu$ and $\nu_2$ fixed, and provided $M_D = \sup_k |D_k| < \infty$ (assumption (iii) below), the sum of the cross-square residual (c) and the leftover from the first Young absorption (Equation~\ref{eq:sat-young}) is bounded by $\bigl[(1 + 1/\mu)\, M_D^2\, |K_\xi|^2\, |W_s|\bigr]\, |\xi_k|^2$. We then choose $\rho$ large enough that the eroded $\xi$-decay dominates this bound,
\begin{equation}
...
\begin{equation}
D_{2,1} = \Delta t\, a_1\, g_{1,k} = \frac{5.1762\, \Delta t\, T_{y,k}}{u_T\, \cs\psi_k},
\qquad
D_{3,1} = \Delta t\, a_5\, g_{2,k} = -\frac{5.1762\, \Delta t\, T_{x,k}}{u_T\, \cs\phi_k\, \cs\psi_k},
\end{equation}
...
M_D \;\leq\; \sqrt{6}\; \Delta t\, \max\!\left( \frac{1}{m},\;\; \frac{5.1762}{\epsilon_u\, \epsilon_a},\;\; \frac{a_3\, l}{J_{xx}},\;\; \frac{a_7\, l}{J_{yy}},\;\; \frac{1}{J_{zz}} \right) \;<\; \infty.
\end{equation}
For the successful-landing trials, Figure~\ref{fig:sat_control_norm_on} shows the applied effort norm holding at the allocation bound through the reaching transient and then relaxing toward hover trim, never approaching zero thrust---evidence for the nonzero-thrust condition of Equation~\ref{eq:sat-envelope}, though not for its attitude conditions. This check is also one-sided: the successful trials are, by selection, exactly those the controller recovers, so the figure shows the envelope is non-vacuous on the recovered set but does not establish that failing trials fail because they leave it. Characterizing where failures exit the envelope is left to future work. We also note that the envelope is needed only to bound the cross-coupling magnitude, not the anti-windup direction on the thrust channel: substituting $D_{1,1} = \Delta t\, \cs\phi_k\, \cs\theta_k / m$ into the wrapped thrust law cancels the attitude factors, giving $\partial u_T / \partial \xi_T = -\eta_z\, \Delta t$, so a positive thrust deficit reduces the commanded thrust by the same amount at every attitude.
...
\end{equation}
At the implemented $1.2741$ scaling only $\xi^\infty$ changes, shrinking to $\xi_\alpha^\infty = c_\alpha/(1.2741\,\eta_\alpha\,\Delta t)$: by Equation~\ref{eq:sat-xi-infty}, $\tilde s^\infty = E^{-1} D\, c$ does not involve $K_\xi$, so every residual ratio below is unchanged. For a deficit on channel $\alpha$, $c = c_\alpha\, e_\alpha$, the same-channel ratio $|\tilde s_i^\infty|/|c_\alpha| = |D_{i\,\alpha}|/(\eta_i \Delta t)$ becomes, substituting from Equation~\ref{eq:sat-D-matrix},
\small
\begin{align}
...
\midrule
Reachability fraction & $0.607$ & $0.221$ \\
Peak success ratio & $0.972$ & $0.500$ \\
Half-success radius (m) & $239.40$ & $203.08$ \\
Centroid $(x, y)$ (m) & $(420.24,\, -257.48)$ & $(414.84,\, -230.05)$ \\
\bottomrule
\end{tabular}
\end{table}
\normalsize
