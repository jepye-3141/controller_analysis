...
**Ballistic-dependent results were regenerated 2026-07-11** on the retuned Vo94/el64 physics -- the sat sweeps / TO figs, the N=41 LUT log + promoted SO log + theta*, the envelope / fig 39, and the figures -- and the paper cites the current numbers (memories `project_release_pass_audit_2026-07-12`, `project_ballistics_e2e_audit_2026-07-12`; the one still-open paper-prose fix is P6 -- the ballistic-success inequality writes the symbol as the body-rate omega_t at `root.tex` ~L886 while the prose says Euler-angle rate, cosmetic, 2026-07-12). A further re-regeneration is pending only to fold in sub-mm post-audit code fixes (behavior-neutral to <0.016%) and the deferred refactors in `docs/refactor_backlog.md`; it will not materially move the numbers.
